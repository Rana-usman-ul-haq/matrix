...
contract FTM is ERC20, Ownable {
    uint256 public buyFeeRate = 1;
    uint256 public sellFeeRate = 2;

    address public taxWallet = 0xE07a5c9055c68bBa0Cc94470Ba54Dc5B0948e843;
    ...
    constructor() ERC20("FUCK THE MATRIX", "FTM") {
        ...
        //EXCLUDE ADDRESSES FROM TAXES
        isWhiteListed[owner()] = true;
        isWhiteListed[taxWallet] = true;
    }
    ...
    function _transfer(
        ...
        if (sender == uniswapV2Pair) {
            if(buyFeeRate > 0) {
            uint256 fee = (amount * buyFeeRate) / 100;
        ...
        if(recipient == uniswapV2Pair) {
            if(sellFeeRate > 0) {
            uint256 fee = (amount * sellFeeRate) / 100;
            transferAmount = amount - fee;
            super._transfer(sender, taxWallet, fee);
            }
        }
    ...
    function changeTaxes(uint256 buyTax, uint256 sellTax) public onlyOwner {
        require(buyTax <= 20, "Tax too high");
        require(sellTax <= 20, "Tax too high");
        buyFeeRate = buyTax;
        sellFeeRate = sellTax;
    }
}
